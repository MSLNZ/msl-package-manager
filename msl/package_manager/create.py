...
def create(names, author=None, email=None):
    ...
    for name in _names:
        if name[0].isdigit():
            print(Fore.YELLOW + 'A package name cannot start with a number -- ignored ' + name)
            continue

        keep = True
        for c in name:
            if not (c.isalnum() or c != '_'):
                print(Fore.CYAN + 'A package name can only contain letters, numbers and underscores -- ignored ' + name)
                keep = False
                break
        ...
        if keep:
            ...
            if os.path.isdir(root):
                print(Fore.MAGENTA + 'A {0} folder already exists. Will not overwrite'.format(msl_name))
            else:
                roots.append(root)
    ...
    if author is not None:
        ...
        try:
            ...
            print(Fore.RED + 'Aborted --  did not create MSL package.')
            return
    ...
    for msl_root, msl_pkg in zip(roots, pkg_names):
        ...
        if os.path.isdir(msl_root):
            ...
            print(Fore.RED + 'Error creating... ' + msl_pkg)
